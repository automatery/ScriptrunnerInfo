...
$parentModule = 'ScriptrunnerInfo'
if (-not $ModuleName) { $ModuleName = 'ScriptrunnerInfo.Client' }
...
$publishRoot = Join-Path -Path $WorkingDirectory -ChildPath 'publish\ScriptrunnerInfo'
...
$originalManifestData = Import-PowerShellDataFile -Path "$publishRoot\ScriptrunnerInfo.psd1"
$prereqHash = @{
